...
const getFilesToUpload = async (bucketName: string, folder: string): Promise<File[]> => {
    ...
    const filteredFiles = (await Promise.all(allFiles.map(f => s3HashNotEquals(f, bucketName).then(ne => ({f, upload: ne})))))
...
const getAllFiles = async (folder: string, baseFolder: string): Promise<File[]> => {
    ...
    const fileDirMap = files.map(file => path.join(folder, file))
        .map(path => ({path, isDir: fs.lstatSync(path).isDirectory()}));

    const subDirFiles: File[] = await Promise.all(fileDirMap.filter(e => e.isDir)
        .map(p => getAllFiles(p.path, baseFolder)))
        .then(files => files.reduce((acc, curr) => [...acc, ...curr], []));

    const directFiles: File[] = await Promise.all(fileDirMap.filter(e => !e.isDir)
